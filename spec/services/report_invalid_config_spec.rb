  ...
  describe ".call" do
    context "given a custom message" do
      it "reports the file as an invalid config file to Github" do
        ...
        expect(commit_status).to have_received(:set_config_error).with(message, nil)
    ...
    context "given a details URL" do
      it "reports the file as an invalid config file to Github with a details link" do
        ...
        expect(commit_status).to have_received(:set_config_error).with(message, details_url)
        expect(Build).to have_received(:find_by!).with(
          pull_request_number: pull_request_number,
          commit_sha: commit_sha,
        )
